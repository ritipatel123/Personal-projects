...
def print_sorted_commits(sorted_commits):
    ...
    for i in range(len(sorted_commits)):
        ...
        next_commit = sorted_commits[i + 1] if i < len(sorted_commits) - 1 else None

        # Print the sticky start if required
        if sticky_start:
            print("=", *current.children)
            sticky_start = False

        # Print the current commit hash and any branch names
        if not current.branches:
            print(current.commit_hash)
        else:
            print(f"{current.commit_hash} ", *current.branches)

        # Determine if a sticky end is needed before moving to the next commit
        if next_commit and next_commit.commit_hash not in current.parents:
